...
import { Shield, Mail, AlertTriangle, ArrowRight, CheckCircle2 } from "lucide-react";
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      if (res && res.error) {
        ...
        setMessage(res.message || "Reset link simulation successful.");
        setLoading(false);
      }
    } catch (err: any) {
    ...
    <div className="relative min-h-screen flex items-center justify-center bg-cyber-bg px-4 overflow-hidden">
      ...
      <div className="w-full max-w-md relative z-10">
        ...
        <div className="flex flex-col items-center mb-8">
          ...
          <span className="text-sm font-mono text-cyber-muted">CREDENTIAL RECOVERY SYSTEM</span>
        </div>

        {/* Card Panel */}
        <div className="glass-panel p-8 rounded-lg shadow-2xl relative">
          <h2 className="text-2xl font-bold text-white mb-6 text-center font-mono">[ PASSWORD_RESET ]</h2>
          ...
          {message ? (
            <div className="mb-6 p-6 bg-emerald-950/30 border border-cyber-success/40 rounded text-center text-cyber-success font-mono flex flex-col items-center space-y-4">
              ...
              <div>
                ...
                <p className="text-xs text-cyber-muted mt-2 leading-relaxed">{message}</p>
                <Link href="/auth/reset-password" className="text-cyber-primary hover:underline text-xs mt-4 block">
            ...
            <>
              ...
              <form onSubmit={handleSubmit} className="space-y-6">
                ...
                >
                  <span>{loading ? "TRANSMITTING..." : "SEND RECOVERY CODE"}</span>

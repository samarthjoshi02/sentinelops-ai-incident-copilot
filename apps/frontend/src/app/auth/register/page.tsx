...
import { Shield, Key, Mail, User, ShieldAlert, AlertTriangle, CheckCircle2 } from "lucide-react";
    ...
    <div className="relative min-h-screen flex items-center justify-center bg-cyber-bg px-4 overflow-hidden">
      ...
      <div className="w-full max-w-md relative z-10 my-10">
        ...
        <div className="flex flex-col items-center mb-8">
          ...
          <span className="text-sm font-mono text-cyber-muted">SECURE ENROLLMENT SYSTEM</span>
        </div>

        {/* Card Panel */}
        <div className="glass-panel p-8 rounded-lg shadow-2xl relative">
          <h2 className="text-2xl font-bold text-white mb-6 text-center font-mono">[ ENROLL_NEW_AGENT ]</h2>
          ...
          {success ? (
            <div className="mb-6 p-6 bg-emerald-950/30 border border-cyber-success/40 rounded text-center text-cyber-success font-mono flex flex-col items-center space-y-4">
              <CheckCircle2 className="w-12 h-12 text-cyber-success animate-bounce" />
              <div>
                <p className="font-bold text-lg">ENROLLMENT COMPLETE</p>
                <p className="text-xs text-cyber-muted mt-1">Redirecting to secure terminal...</p>
              </div>
            </div>
          ) : (
            <>
              ...
              <form onSubmit={handleSubmit} className="space-y-5">
                <div>
                  <label className="block text-xs font-mono text-cyber-muted uppercase tracking-wider mb-2">Operative Name</label>
                  <div className="relative">
                    <User className="absolute left-3 top-1/2 transform -translate-y-1/2 w-4 h-4 text-cyber-muted" />
                    <input
                      type="text"
                      name="name"
                      required
                ...
                <div>
                  <label className="block text-xs font-mono text-cyber-muted uppercase tracking-wider mb-2">Corporate Email</label>
                  <div className="relative">
                ...
                <div>
                  <label className="block text-xs font-mono text-cyber-muted uppercase tracking-wider mb-2">Security Password</label>
                  <div className="relative">
                ...
                <div>
                  <label className="block text-xs font-mono text-cyber-muted uppercase tracking-wider mb-2">Assign Operational Role</label>
                  <div className="relative">
                    <ShieldAlert className="absolute left-3 top-1/2 transform -translate-y-1/2 w-4 h-4 text-cyber-muted" />
                    <select
                      name="role"
                      defaultValue="READ_ONLY"
                      className="w-full pl-10 pr-4 py-3 text-sm rounded glass-input transition-all duration-200 appearance-none cursor-pointer"
                    >
                ...
                <button
                  type="submit"
                  disabled={loading}
                  className="w-full py-3 px-4 rounded text-sm font-mono font-bold text-white glow-btn flex items-center justify-center space-x-2 disabled:opacity-50 disabled:cursor-not-allowed mt-8"
                >
                  <span>{loading ? "ENROLLING AGENT..." : "ENROLL NEW AGENT"}</span>
                </button>
              </form>
            </>
          )}
